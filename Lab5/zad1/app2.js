...
app.get('/', function (req, res) {      // The first route
    let str = String(x + " + " + y + " = " + (x+y));
    res.cookie('cookie', String(x+y));
    res.render('index', {pretty:true, str:str}); // Render the 'index' view in 'pretty' mode - the resulting HTML code will be indented - the 'pretty' option has the 'deprecated' status - in the future it will not be supported
...
app.get('/add/:a/:b', function (req, res) {     // The first route
    let str = String(x + " + " + y + " = " + (x+y));
    res.render('index', {pretty:true, str:str});
    console.log(str);
});
